...
function matrixSum(matrix) {
  ...
  var recurse = function(rowsFound, colsFound, currSum, found) {
  if (found === rows) {
    if (currSum > maxSum) {
      maxSum = currSum;
    }
  ...
  for (var i = 0; i<rows; i++) {
    for (var j = 0; j < cols; j++) {
      if (!rowsFound[i] && !colsFound[j]) {
        currSum += matrix[i][j];
        recurse(rowsFound, colsFound, currSum);
        found++;
      }
    }
  }
  ...
  recurse(rowsFoundGlobal, colsFoundGlobal, currSumGlobal, foundGlobal);
  
  return maxSum;
}

console.log(matrixSum(smallMatrix))
